...
/**
 * The OpenGL renderer implementation in JOGL4
 * For compatible hardware this allows program_binary to cache shader binary blobs for performance.
 * @author Charlie
 * 
 */
public class RendererGL4 implements GLEventListener {
	...
	public void generateRenderBuffers(GL4 gl){
		//Generate the buffers required for deferred lighting
	}
	
	public void loadCore(GL4 gl){
	...
	public void display(GLAutoDrawable arg0) {
		...
		//gl.glEnableClientState(GL4.GL_VERTEX_ARRAY);
		gl.glVertexAttribPointer(vboLocation, 2, GL.GL_FLOAT, false, 0,0);//2=xy
		gl.glEnableVertexAttribArray(vboLocation);
		...
		if(Graphics.current.useFbo){
			...
			int[] buffers = {GL4.GL_COLOR_ATTACHMENT0, GL4.GL_COLOR_ATTACHMENT1, GL4.GL_COLOR_ATTACHMENT2};
...
@Override
	...
	private void createFbos(GL4 gl) {
		...
		for(int i = 0; i<3; i++){
			...
			//gl.glTexParameteri(GL.GL_TEXTURE_2D, GL4.GL_GENERATE_MIPMAP, GL.GL_TRUE); // automatic mipmap
			
			//WORKS	LDR gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_RGBA8, width, height, 0, GL4.GL_RGBA, GL.GL_UNSIGNED_BYTE, null);
			
			//int type = (Graphics.current.hdr?GL4.GL_RGBA32F:GL.GL_RGBA8);//RGBA16F
			int type = GL.GL_RGBA8;
			gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, type, Graphics.camera.width, Graphics.camera.height, 0, GL4.GL_RGBA, GL.GL_UNSIGNED_BYTE, null);
		}
		...
		gl.glTexParameteri(GL.GL_TEXTURE_2D, GL.GL_DEPTH_TEXTURE_MODE, GL4.GL_LUMINANCE);//when read as tex interpret as luminance. can be alpha
		//gl.glTexParameteri(GL.GL_TEXTURE_2D, GL4.GL_TEXTURE_COMPARE_MODE, GL4.GL_COMPARE_R_TO_TEXTURE);
		gl.glTexParameteri(GL.GL_TEXTURE_2D, GL4.GL_TEXTURE_COMPARE_FUNC, GL4.GL_LEQUAL);

		gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_DEPTH_COMPONENT32, Graphics.camera.width, Graphics.camera.height, 0, GL4.GL_DEPTH_COMPONENT, GL.GL_UNSIGNED_INT, null);
		...
		gl.glFramebufferTexture2D(GL.GL_FRAMEBUFFER, GL4.GL_COLOR_ATTACHMENT0, GL.GL_TEXTURE_2D, deferredTargets[0], 0 );
		gl.glFramebufferTexture2D(GL.GL_FRAMEBUFFER, GL4.GL_COLOR_ATTACHMENT1, GL.GL_TEXTURE_2D, deferredTargets[1], 0 );
		gl.glFramebufferTexture2D(GL.GL_FRAMEBUFFER, GL4.GL_COLOR_ATTACHMENT2, GL.GL_TEXTURE_2D, deferredTargets[2], 0 );
		gl.glFramebufferTexture2D(GL.GL_FRAMEBUFFER, GL4.GL_DEPTH_ATTACHMENT, GL.GL_TEXTURE_2D, depthtex, 0 );
	...
	public void reshape(GLAutoDrawable arg0, int arg1, int arg2, int arg3,
		...
		for(int i = 0; i<3; i++){
			int fbotex = deferredTargets[i];
			gl.glBindTexture(GL.GL_TEXTURE_2D, fbotex);
			gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, GL.GL_RGBA8, Graphics.camera.width, Graphics.camera.height, 0, GL4.GL_RGBA, GL.GL_UNSIGNED_BYTE, null);
		}
	}
	
	public void loadTextures(String[] filenames, GL4 gl){
		...
		for(int i=0;i<fn.length; i++)
		{
			...
			try{
				...
				gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, d.getInternalFormat(), d.getWidth(), d.getHeight(), 0, d.getPixelFormat()/*GL4.GL_RGBA/**/, GL.GL_UNSIGNED_BYTE, d.getBuffer());
	...
	private int newSingleFixedTexture(String string, GL4 gl) {
		...
		try {
            ...
            gl.glTexImage2D(GL.GL_TEXTURE_2D, 0, data.getInternalFormat(), data.getWidth(), data.getHeight(), 0, data.getPixelFormat()/*GL4.GL_RGBA/**/, GL.GL_UNSIGNED_BYTE, data.getBuffer());
	...
	private int loadTexture1D(String string, GL4 gl) {
		int[] texid = new int[1];
		...
		try {
           ...
           //TODO BUG FIXME Uses a hack to convert RGB to BGR in the shader. SHIIIIIIIT.
            ...
            gl.glBindTexture(GL4.GL_TEXTURE_1D, texid[0]);
            
            //Assuming we are just loading CC ramps this is awesome.
            
            gl.glTexParameterf(GL4.GL_TEXTURE_1D, GL.GL_TEXTURE_MIN_FILTER, GL.GL_NEAREST);
            gl.glTexParameterf(GL4.GL_TEXTURE_1D, GL.GL_TEXTURE_MAG_FILTER, GL.GL_NEAREST);
    		gl.glTexParameterf(GL4.GL_TEXTURE_1D, GL.GL_TEXTURE_WRAP_S, GL.GL_CLAMP_TO_EDGE);
    		gl.glTexParameterf(GL4.GL_TEXTURE_1D, GL.GL_TEXTURE_WRAP_T, GL.GL_CLAMP_TO_EDGE);
    		
            gl.glTexImage1D(GL4.GL_TEXTURE_1D, 0, data.getInternalFormat(), data.getWidth(), 0/*border*/, data.getPixelFormat(), GL.GL_UNSIGNED_BYTE, data.getBuffer());
	...
	public int texid(String filename, GL4 gl){
